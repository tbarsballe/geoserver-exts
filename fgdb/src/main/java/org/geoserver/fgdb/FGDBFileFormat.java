...
public class FGDBFileFormat extends VectorFormat {
        ...
        if( data instanceof FileData ){
            ...
            for( Name name : names ){
                try {
                    ...
                    LOGGER.log(Level.WARNING,"Unable to access "+name+" for import:"+issue, issue);
                }
            }
        }
        return tasks;
     ...
     * 
     * @param store OGRDataStore
     * @param catalog 
     * @param name
     * @return import task for aynchronous loading
     */
    private ImportTask task(ImportData data, Catalog catalog, FeatureSource<FeatureType, Feature> source) throws IOException {
        //
